...
def set_logger(logger_name):
    ...
    os.makedirs(f'log/{logger_file_name}', exist_ok=True)#注意，windows中需调整格式 #bug0：未使用基于时间的logger文件名

    # 创建按天轮转的日志处理器
    fh = TimedRotatingFileHandler(filename=f'log/{logger_name}/log', when='D', interval=1, backupCount=1)#TimedRotatingFileHandler按天轮转（when='D'），1文件/day，保留一个备份
...
def data_process(username, request_operation, json_data, connection_socket):
    ...
    #处理SAVE操作
    if request_operation == OP_SAVE: #bug13:采用小写的op_save，而非正确大写（python大小写分别处理）
...
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_SAVE:
        ...
        if os.path.exists(join('file', username, key)) is True:
            ...
            return
        #bug15：状态码对应错误，此处缺失size字段，应为Error410：字段缺失
    ...
    if request_operation == OP_UPLOAD:
        ...
        if FIELD_BLOCK_INDEX not in json_data.keys(): #bug16：filed_block_index为必须字段，必需字段缺失为Error 400
    ...
    if request_operation == OP_DOWNLOAD:
        ...
        if block_index >= total_block:
            logger.error(f'<-- The "block_index" exceed the max index.')
            connection_socket.send( #bug17,块索引超界，错误代码应为405
    ...
    while True:
        ...
        for _compulsory_fields in compulsory_fields:
            if _compulsory_fields not in list(json_data.keys()):
                ...
                check_ok = False
                break
        if check_ok : #bug9：逻辑错误，check_ok=false时，说明其触发error400，退出函数
            continue
        ...
        token: str
        #Error403，token格式错误
        if len(token.split('.')) != 3: #bug10,token格式为（用户名.时间戳.MD5加密认证值），长度为3
            connection_socket.send(
                make_response_packet(request_operation, 403, TYPE_AUTH, f'Token format is wrong.', {}))
            continue

        user_str = ".".join(token.split('.')[:2])#bug11，原理同bug10
        md5_auth_str = token.split('.')[2]#bug12，原理同bug10
